...
fun HorizontalDottedProgressBar() {
    ...
    val state by transition.animateFloat(
        initialValue = 0f, targetValue = 6f, animationSpec = infiniteRepeatable(
            animation = tween(
                durationMillis = 700,
                easing = LinearEasing
            )
...
fun DrawCanvas(state: Float, color: Color) {
    Canvas(
        ...
        for (i in 1..5) {
            if (i - 1 == state.toInt()) {
                drawCircle(
                    radius = radius * 2, brush = SolidColor(color), center = Offset(
                ...
                drawCircle(
                    brush = SolidColor(color), radius = radius, center = Offset(
                        x = this.center.x + radius * 2 * (i - 3) + padding * (i - 3),
                        y = this.center.y,
                    )
                )
